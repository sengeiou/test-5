...
int ITc_eom_get_output_id_p(void)
{
	...
	int id_cnt = -1;
	eom_output_id* output_ids = eom_get_eom_output_ids(&id_cnt);
...
int ITc_eom_get_output_type_p(void)
{
	...
	for ( enum_counter=0; enum_counter<id_cnt; enum_counter++ )
	{
		nRet = eom_get_output_type(output_ids[enum_counter], &type);
...
int ITc_eom_get_output_mode_p(void)
{
	...
	for ( enum_counter=0; enum_counter<id_cnt; enum_counter++ )
	{
		nRet = eom_get_output_mode(output_ids[enum_counter], &mode);
...
int ITc_eom_get_set_output_attribute_p(void)
{
	...
	for ( enum_counter=0;enum_counter<id_cnt;enum_counter++)
	{
		attribute1 = attribute[enum_counter];
		nRet = eom_set_output_attribute(output_ids[enum_counter], attribute1);
		PRINT_RESULT_CLEANUP(EOM_ERROR_NONE, nRet,"eom_set_output_attribute", EomGetError(nRet), FREE_MEMORY(output_ids));
		sleep(2);
		
		nRet = eom_get_output_attribute(output_ids[enum_counter], &attribute2);
...
int ITc_eom_get_output_attribute_state_p(void)
{
	...
	for ( enum_counter=0; enum_counter<id_cnt; enum_counter++ )
	{
		nRet = eom_get_output_attribute_state(output_ids[enum_counter], &state);
...
int ITc_eom_get_output_resolution_p(void)
{
	...
	for ( enum_counter=0; enum_counter<id_cnt; enum_counter++ )
	{
		nRet = eom_get_output_resolution(output_ids[enum_counter], &width, &height);
...
int ITc_eom_get_output_physical_size_p(void)
{
	...
	for ( enum_counter=0; enum_counter<id_cnt; enum_counter++ )
	{
		nRet = eom_get_output_physical_size(output_ids[enum_counter], &phy_width,&phy_height);
...
int ITc_eom_set_output_window_p(void)
{
	...
	for ( enum_counter=0; enum_counter<id_cnt; enum_counter++ )
	{
		nRet = eom_set_output_attribute(output_ids[enum_counter], attribute);
		PRINT_RESULT_CLEANUP(EOM_ERROR_NONE, nRet, "eom_set_output_attribute", EomGetError(nRet), FREE_MEMORY(output_ids));
		
		nRet = eom_set_output_window(output_ids[enum_counter], win);
...
* @apicovered			eom_get_eom_output_ids, eom_set_output_added_cb, eom_unset_output_added_cb					
* @passcase				When eom_get_eom_output_ids and eom_set_output_added_cb are successful
* @failcase				If target API fails
* @precondition			NA
* @postcondition		NA
*/
...
* @apicovered			eom_get_eom_output_ids, eom_set_output_removed_cb,eeom_unset_output_removed_cb					
* @passcase				When eom_get_eom_output_ids and eom_set_output_removed_cb are successful
* @failcase				If target API fails
* @precondition			NA
* @postcondition		NA
*/
