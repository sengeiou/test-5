...
UTC_HIDDEN int utc_calendar_db_get_records_with_query_p_todo_book(void)
{
	...
	ret = calendar_query_create(_calendar_todo_calendar_book._uri, &query);
	assert_eq(ret, CALENDAR_ERROR_NONE);

	calendar_filter_h filter = NULL;
	ret = calendar_filter_create(_calendar_todo_calendar_book._uri, &filter);
	if (CALENDAR_ERROR_NONE != ret) {
		calendar_query_destroy(query);
	}
	assert_eq(ret, CALENDAR_ERROR_NONE);

	ret = calendar_filter_add_int(filter, _calendar_todo_calendar_book.todo_id,
...
UTC_HIDDEN int utc_calendar_db_get_count_with_query_p_todo_book(void)
{
	assert_eq(g_startup_err, CALENDAR_ERROR_NONE);
	int ret = 0;
...
UTC_HIDDEN int utc_calendar_db_get_count_with_query_n_todo_book(void)
{
	return 0;
}
