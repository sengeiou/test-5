...
static bool sync_supported = true;
...
int utc_sync_adapter_set_callbacks_p(void)
{
	...
	else { assert_eq(ret, SYNC_ERROR_NONE); }
...
int utc_sync_adapter_set_callbacks_n(void)
{
	...
	if (!sync_supported) { assert_eq(ret, SYNC_ERROR_NOT_SUPPORTED); }
	else { assert_eq(ret, SYNC_ERROR_INVALID_PARAMETER); }
...
{
	system_info_get_platform_bool(SYNC_FEATURE, &sync_supported);
...
int utc_sync_manager_on_demand_sync_job_n4(void)
{
	...
	ret = sync_manager_on_demand_sync_job(account, name_on_demand, SYNC_OPTION_NONE, user_data, &sync_job_id);
	if (!sync_supported) { assert_eq(ret, SYNC_ERROR_NOT_SUPPORTED); }
	else { assert_eq(ret, SYNC_ERROR_SYNC_ADAPTER_NOT_FOUND); }
