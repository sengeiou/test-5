...
int utc_stt_start_p(void)
{
	...
	if (false == g_stt_supported) {
		...
		ret = stt_set_private_data(g_stt, "stt_verification", "true");
		assert_eq(ret, STT_ERROR_NONE);

		ret = stt_get_default_language(g_stt, &g_language);
		assert_eq_with_reset_TC_mode(ret, STT_ERROR_NONE);
		...
		ret = stt_set_private_data(g_stt, "stt_verification", "false");
		assert_eq(ret, STT_ERROR_NONE);
