...
/**
 * @function       utc_webkit2_ewk_manifest_icons_src_get_func_startup
 * @description    Called before each test
 * @parameter      NA
 * @return         NA
 */
void utc_webkit2_ewk_manifest_icons_src_get_func_startup(void)
{
...
/**
 * @function       utc_webkit2_ewk_manifest_icons_src_get_func_cleanup
 * @description    Called after each test
 * @parameter      NA
 * @return         NA
 */
void utc_webkit2_ewk_manifest_icons_src_get_func_cleanup(void)
{
...
static void DidReceiveRequestManifest(Evas_Object* view, Ewk_View_Request_Manifest* manifest, void* user_data) {
  if (manifest != NULL && !strcmp(ewk_manifest_icons_src_get(manifest, 0), "https://googlechrome.github.io/samples/web-application-manifest/launcher-icon-0-75x.png"))
    isSucceed = EINA_TRUE;
  else
...
static void DidReceiveRequestManifestForNeg(Evas_Object* view, Ewk_View_Request_Manifest* manifest, void* user_data) {
  // Get the src of icons out of range index, and if it returns nullptr it is normal operation.
  if (manifest != NULL && ewk_manifest_icons_src_get(manifest, 7) == NULL)
    isSucceed = EINA_TRUE;
  else
    isSucceed = EINA_FALSE;
...
/**
 * @testcase       utc_webkit2_ewk_manifest_icons_src_get_p
 * @since_tizen    3.0
 ...
 */
int utc_webkit2_ewk_manifest_icons_src_get_p(void)
{
  // 1. Open page.
...
/**
 * @testcase       utc_webkit2_ewk_manifest_icons_src_get_n
 * @since_tizen    3.0
 * @description    To get the src of icons out of range index.
 * @scenario       Load test html page(ewk_view_url_set).
 *                 Execute ewk_view_request_manifest.
 *                 Register callback function and wait until you receive manifest data.
 *                 Get the src of icons out of range index.
 */
int utc_webkit2_ewk_manifest_icons_src_get_n(void)
{
  // 1. Open page.
  assert_eq(EINA_TRUE, loadURL("https://googlechrome.github.io/samples/web-application-manifest"));
  utc_webkit2_main_loop_begin();
