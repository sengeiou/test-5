...
static void intercept_request_callback(Evas_Object* o, Ewk_Intercept_Request* intercept_request, void* user_data) {
  url = (char*)ewk_intercept_request_url_get(null_intercept_request ? NULL : intercept_request);
  if (url)
    url = strdup(url);
}

/**
 * @function		utc_webkit2_ewk_intercept_request_url_get_func_startup
 * @description		Called before each test
 * @parameter		NA
 * @return			NA
 */
void utc_webkit2_ewk_intercept_request_url_get_func_startup(void)
{
...
/**
 * @function		utc_webkit2_ewk_intercept_request_url_get_func_cleanup
 * @description		Called after each test
 * @parameter		NA
 * @return			NA
 */
void utc_webkit2_ewk_intercept_request_url_get_func_cleanup(void)
{
  free(url);
...
/**
 * @testcase		utc_webkit2_ewk_intercept_request_url_get_p
 * @since_tizen		3.0
 ...
 *                  Check if right url was returned
 */
int utc_webkit2_ewk_intercept_request_url_get_p(void)
{
  ...
  assert(url);
  assert(strcmp(url, COMMON_SAMPLE_HTML_FILE) == 0);

  return 0;
}

/**
 * @testcase		utc_webkit2_ewk_intercept_request_url_get_n
 * @since_tizen		3.0
 * @description		Set callback for intercepting requests
 * @scenario		Check operation for entering incorrect parameter
 */
int utc_webkit2_ewk_intercept_request_url_get_n(void)
{
