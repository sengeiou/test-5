...
/**
 * @function       utc_webkit2_ewk_manifest_icons_count_get_func_startup
 * @description    Called before each test
 * @parameter      NA
 * @return         NA
 */
void utc_webkit2_ewk_manifest_icons_count_get_func_startup(void)
{
...
/**
 * @function       utc_webkit2_ewk_manifest_icons_count_get_func_cleanup
 * @description    Called after each test
 * @parameter      NA
 * @return         NA
 */
void utc_webkit2_ewk_manifest_icons_count_get_func_cleanup(void)
{
...
static void DidReceiveRequestManifest(Evas_Object* view, Ewk_View_Request_Manifest* manifest, void* user_data) {
  if (manifest != NULL && ewk_manifest_icons_count_get(manifest) == 6)
    isSucceed = EINA_TRUE;
  else
...
/**
 * @testcase       utc_webkit2_ewk_manifest_icons_count_get_p
 * @since_tizen    3.0
 ...
 */
int utc_webkit2_ewk_manifest_icons_count_get_p(void)
{
  // 1. Open page
  ...
  // If the count of icons is received some value from the manifest, it is normal operation.
  assert_eq(EINA_TRUE, isSucceed);
  return 0;
}

/**
 * @testcase       utc_webkit2_ewk_manifest_icons_count_get_n
 * @since_tizen    3.0
 * @description    To get the count of icons from NULL object.
 * @scenario       Load test html page(ewk_view_url_set).
 *                 Execute ewk_view_request_manifest.
 *                 Register callback function and wait until you receive manifest data.
 *                 Get the count of icons
 */
int utc_webkit2_ewk_manifest_icons_count_get_n(void)
{
  // 1. Open page.
  ...
  // 3. Get the count of icons from NULL object, and if it returns 0 it is normal operation.
  if (ewk_manifest_icons_count_get(NULL) == 0)
    assert(EINA_TRUE);
  else
    assert(EINA_FALSE);
