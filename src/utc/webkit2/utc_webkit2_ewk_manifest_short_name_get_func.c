...
/**
 * @function       utc_webkit2_ewk_manifest_short_name_get_func_startup
 * @description    Called before each test
 * @parameter      NA
 * @return         NA
 */
void utc_webkit2_ewk_manifest_short_name_get_func_startup(void)
{
...
/**
 * @function       utc_webkit2_ewk_manifest_short_name_get_func_cleanup
 * @description    Called after each test
 * @parameter      NA
 * @return         NA
 */
void utc_webkit2_ewk_manifest_short_name_get_func_cleanup(void)
{
...
static void DidReceiveRequestManifest(Evas_Object* view, Ewk_View_Request_Manifest* manifest, void* user_data) {
  if (manifest != NULL && !strcmp(ewk_manifest_short_name_get(manifest),"Capture QR Code"))
    isSucceed = EINA_TRUE;
  else
...
/**
 * @testcase       utc_webkit2_ewk_manifest_short_name_get_p
 * @since_tizen    3.0
 ...
 */
int utc_webkit2_ewk_manifest_short_name_get_p(void)
{
  // 1. Open page.
  assert_eq(EINA_TRUE, loadURL("https://qrsnapper.appspot.com"));
...
/**
 * @testcase       utc_webkit2_ewk_manifest_short_name_get_n
 * @since_tizen    3.0
 * @description    To get short name from NULL object.
 * @scenario       Load test html page(ewk_view_url_set).
 *                 Execute ewk_view_request_manifest.
 *                 Register callback function and wait until you receive manifest data.
 *                 Get short name from NULL object
 */
int utc_webkit2_ewk_manifest_short_name_get_n(void)
{
  // 1. Open page.
  ...
  // 3. Get the short name from NULL object, and if it returns nullptr it is normal operation.
  if (ewk_manifest_short_name_get(NULL) == NULL)
    assert(EINA_TRUE);
  else
    assert(EINA_FALSE);
