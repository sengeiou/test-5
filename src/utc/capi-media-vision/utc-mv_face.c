...
static int init_face_examples()
{
    int err = init_examples_by_dir(p1_face_examples_dir, p1_face_examples_n, &p1_face_examples);
    if (MEDIA_VISION_ERROR_NONE != err)
    {
        printf("Examples set from [%s] was loaded incorrectly!\n", p1_face_examples_dir);
        goto _ERR;
    }

    err = init_examples_by_dir(p2_face_examples_dir, p2_face_examples_n, &p2_face_examples);
    if (MEDIA_VISION_ERROR_NONE != err)
    {
        printf("Examples set from [%s] was loaded incorrectly!\n", p2_face_examples_dir);
    }
...
static int free_face_examples()
{
    int err = free_examples_set(p1_face_examples, p1_face_examples_n);
    if (MEDIA_VISION_ERROR_NONE != err)
    {
        printf("Examples set 1 (from %s) wasn't freed!\n", p1_face_examples_dir);
    }

    err = free_examples_set(p2_face_examples, p2_face_examples_n);
    if (MEDIA_VISION_ERROR_NONE != err)
    {
        printf("Examples set 2 (from %s) wasn't freed!\n", p2_face_examples_dir);
    }
...
void utc_capi_media_vision_face_startup(void)
{
    ...
    if (true == GetValueForTCTSetting("DEVICE_SUITE_TARGET_30", pszValue, API_NAMESPACE)) {
        ...
        track_prep_frame_path=(char*)calloc(strlen(pszValue)+strlen("/res/res/track/sequence/01.jpg")+1, sizeof(char));
        snprintf(track_prep_frame_path, strlen(pszValue)+strlen("/res/res/track/sequence/01.jpg")+1, "%s/res/res/track/sequence/01.jpg", pszValue);
    ...
    if (NULL == tracking_model)
    {
        ...
        {
            tracking_model = NULL;
            printf("Global tracking model has NOT been created.\n");
        }
        else
        {
            printf("Global tracking model has been created.\n");
        }
    }
...
int utc_mediavision_mv_face_track_p(void)
{
    ...
    if (NULL != dir)
    {
        while (NULL != (ent = readdir (dir)))
        {
            ...
            snprintf(frames[frames_counter - 1], FILE_PATH_SIZE, "%s/%s",
                                              track_seq_path, ent->d_name);
        }
        closedir(dir);
    }

    // needed at least two frames: one for preparation and another for track
    if (frames_counter < 2) {
        printf("Error occured in preparing two frames for tracking\n");
        free(frames);
        return 1;
    }
    ...
    if (0 != utc_mediavision_mv_face_tracking_model_prepare_p()) {
        printf("Error occured in face_tracking_model_prepare()\n");
        free(frames);
        return 1;
    }
    ...
    ret = mv_create_source(&source);
    if (source == NULL) {
        printf("Error occured in mv_create_source()\n");
        free(frames);
        return 1;
    }
    ...
    {
        ret = load_image_to_media_source(frames[idx], source);
        if (ret != MEDIA_VISION_ERROR_NONE) {
...
_ERR_ :
    free(frames);
    if (source != NULL) {
        retTmp = mv_destroy_source(source);
        printf("ret value: %d\n", retTmp);
    }
...
int utc_mediavision_mv_face_recognition_model_clone_p(void)
{
    ...
    assert_eq(MEDIA_VISION_ERROR_NONE, init_face_examples());
    ...
    assert_eq(MEDIA_VISION_ERROR_NONE, free_face_examples());
...
int utc_mediavision_mv_face_detect_p(void)
{
    ...
    number_of_faces_on_image = 2;
    is_face_detected_cb_called = false;
    ret = mv_face_detect(source2, NULL, face_detected_cb, &number_of_faces_on_image);
    assert_eq(MEDIA_VISION_ERROR_NONE, ret);
    assert_eq(true, is_face_detected_cb_called);

    number_of_faces_on_image = 3;
    is_face_detected_cb_called = false;
    ret = mv_face_detect(source3, NULL, face_detected_cb, &number_of_faces_on_image);
    assert_eq(MEDIA_VISION_ERROR_NONE, ret);
    assert_eq(true, is_face_detected_cb_called);

    number_of_faces_on_image = 6;
    is_face_detected_cb_called = false;
    ret = mv_face_detect(source4, NULL, face_detected_cb, &number_of_faces_on_image);
    assert_eq(MEDIA_VISION_ERROR_NONE, ret);
    assert_eq(true, is_face_detected_cb_called);
