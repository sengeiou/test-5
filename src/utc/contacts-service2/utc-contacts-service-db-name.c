...
#include "utc-contacts-service-db-name.h"
...
ctsvc_tc_db_plugin_s _tc_db_name = {
	.db_add_changed_cb = _ctsvc_tc_db_add_changed_cb_name,
	.db_remove_changed_cb = _ctsvc_tc_db_remove_changed_cb_name,
	.db_get_record = _ctsvc_tc_db_get_record_name,
	.db_get_records_with_query = _ctsvc_tc_db_get_records_with_query_name,
	.db_get_all_records = _ctsvc_tc_db_get_all_records_name,
	.db_get_changes_by_version = NULL,
	.db_get_count = _ctsvc_tc_db_get_count_name,
	.db_get_count_with_query = _ctsvc_tc_db_get_count_with_query_name,
	.db_insert_record = _ctsvc_tc_db_insert_record_name,
	.db_insert_records = _ctsvc_tc_db_insert_records_name,
	.db_update_record = _ctsvc_tc_db_update_record_name,
	.db_update_records = _ctsvc_tc_db_update_records_name,
	.db_delete_record = _ctsvc_tc_db_delete_record_name,
	.db_delete_records = _ctsvc_tc_db_delete_records_name,
...
static int _ctsvc_tc_db_add_changed_cb_name(void)
{
	int ret = 0;
	do {
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_add_changed_cb(_contacts_name._uri, ctsvc_tc_util_db_changed_cb, NULL))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_remove_changed_cb(_contacts_name._uri, ctsvc_tc_util_db_changed_cb, NULL))) break;
		return 0;
	} while (0);
	return ret;
}
...
static int _ctsvc_tc_db_get_record_name(int account_id)
{
	...
	do {
		...
		if (0 != (ret = ctsvc_tc_record_create_name(contact_id, PROPERTY_TYPE_CREATE1, &record))) break;
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_record(_contacts_name._uri, id, &record))) break;
		if (0 != (ret = ctsvc_tc_record_compare_name(record, PROPERTY_TYPE_CREATE1))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_record_destroy(record, true))) break;
		record = NULL;

		if (CONTACTS_ERROR_NONE != (ret = contacts_db_delete_record(_contacts_name._uri, id))) break;
	...
	contacts_db_delete_record(_contacts_name._uri, id);
...
static int _ctsvc_tc_db_get_records_with_query_name(int account_id)
{
	...
	do {
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_query_create(_contacts_name._uri, &query))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_filter_create(_contacts_name._uri, &filter))) break;
		if (0 != (ret = ctsvc_tc_filter_set_name(filter, PROPERTY_TYPE_CREATE1))) break;
		...
		if (0 != (ret = ctsvc_tc_record_compare_name(record_p, PROPERTY_TYPE_CREATE1))) break;
...
static int _ctsvc_tc_db_get_all_records_name(int account_id)
{
	...
	do {
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_all_records(_contacts_name._uri, OFFSET_DEFAULT, LIMIT_DEFAULT, &list))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_list_get_current_record_p(list, &record_p))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_list_destroy(list, true))) break;
		list = NULL;
...
static int _ctsvc_tc_db_get_count_name(int account_id)
{
	...
	do {
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_count(_contacts_name._uri, &count))) break;
		if (count < 1) {
			ret = -1;
			break;
		}
...
static int _ctsvc_tc_db_insert_records_name(int account_id)
{
	...
	do {
		...
		if (0 != (ret = ctsvc_tc_db_insert_record_empty_contact(address_book_id, group_id, &contact_id2))) break;

		if (CONTACTS_ERROR_NONE != (ret = contacts_list_create(&list))) break;
		if (0 != (ret = ctsvc_tc_record_create_name(contact_id1, PROPERTY_TYPE_CREATE1, &record))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_list_add(list, record))) break;
		record = NULL;
		if (0 != (ret = ctsvc_tc_record_create_name(contact_id2, PROPERTY_TYPE_CREATE2, &record))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_list_add(list, record))) break;
		record = NULL;
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_record(_contacts_name._uri, ids[0], &record))) break;
		if (0 != (ret = ctsvc_tc_record_compare_name(record, PROPERTY_TYPE_CREATE1))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_record_destroy(record, true))) break;
		record = NULL;

		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_record(_contacts_name._uri, ids[1], &record))) break;
		if (0 != (ret = ctsvc_tc_record_compare_name(record, PROPERTY_TYPE_CREATE2))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_record_destroy(record, true))) break;
		record = NULL;

		if (CONTACTS_ERROR_NONE != (ret = contacts_db_delete_records(_contacts_name._uri, ids, ids_count))) break;
		free(ids);
		ids = NULL;
		ids_count = 0;

		if (CONTACTS_ERROR_NONE != (ret = contacts_db_delete_record(_contacts_contact._uri, contact_id1))) break;
	...
	contacts_record_destroy(record, true);
	contacts_list_destroy(list, true);
	contacts_db_delete_records(_contacts_name._uri, ids, ids_count);
	free(ids);
	contacts_db_delete_record(_contacts_contact._uri, contact_id1);
...
static int _ctsvc_tc_db_update_record_name(int account_id)
{
	...
	do {
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_record(_contacts_name._uri, id, &record))) break;
		if (0 != (ret = ctsvc_tc_record_update_name(record, PROPERTY_TYPE_UPDATE1))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_update_record(record))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_record_destroy(record, true))) break;
		record = NULL;

		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_record(_contacts_name._uri, id, &record))) break;
		if (0 != (ret = ctsvc_tc_record_compare_name(record, PROPERTY_TYPE_UPDATE1))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_record_destroy(record, true))) break;
		record = NULL;
...
static int _ctsvc_tc_db_update_records_name(int account_id)
{
	...
	do {
		...
		record = NULL;
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_record(_contacts_name._uri, ids[1], &record))) break;
		if (0 != (ret = ctsvc_tc_record_update_name(record, PROPERTY_TYPE_UPDATE2))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_list_add(list, record))) break;
		record = NULL;
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_record(_contacts_name._uri, ids[1], &record))) break;
		if (0 != (ret = ctsvc_tc_record_compare_name(record, PROPERTY_TYPE_UPDATE2))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_record_destroy(record, true))) break;
		record = NULL;
...
static int _ctsvc_tc_db_delete_record_name(int account_id)
{
	...
	do {
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_delete_record(_contacts_name._uri, id))) break;
		if (CONTACTS_ERROR_NO_DATA != (ret = contacts_db_get_record(_contacts_name._uri, id, &record))) {
			ret = -1;
			break;
		}
...
static int _ctsvc_tc_db_delete_records_name(int account_id)
{
	...
	do {
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_delete_records(_contacts_name._uri, ids, ids_count))) break;
		if (CONTACTS_ERROR_NO_DATA != (ret = contacts_db_get_record(_contacts_name._uri, ids[0], &record))) {
			ret = -1;
			break;
		}
		if (CONTACTS_ERROR_NO_DATA != (ret = contacts_db_get_record(_contacts_name._uri, ids[1], &record))) {
			ret = -1;
			break;
		}
		free(ids);
