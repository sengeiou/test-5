...
#include "utc-contacts-service-db-event.h"
...
ctsvc_tc_db_plugin_s _tc_db_event = {
	.db_add_changed_cb = _ctsvc_tc_db_add_changed_cb_event,
	.db_remove_changed_cb = _ctsvc_tc_db_remove_changed_cb_event,
	.db_get_record = _ctsvc_tc_db_get_record_event,
	.db_get_records_with_query = _ctsvc_tc_db_get_records_with_query_event,
	.db_get_all_records = _ctsvc_tc_db_get_all_records_event,
	.db_get_changes_by_version = NULL,
	.db_get_count = _ctsvc_tc_db_get_count_event,
	.db_get_count_with_query = _ctsvc_tc_db_get_count_with_query_event,
	.db_insert_record = _ctsvc_tc_db_insert_record_event,
	.db_insert_records = _ctsvc_tc_db_insert_records_event,
	.db_update_record = _ctsvc_tc_db_update_record_event,
	.db_update_records = _ctsvc_tc_db_update_records_event,
	.db_delete_record = _ctsvc_tc_db_delete_record_event,
	.db_delete_records = _ctsvc_tc_db_delete_records_event,
...
static int _ctsvc_tc_db_add_changed_cb_event(void)
{
	int ret = 0;
	do {
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_add_changed_cb(_contacts_event._uri, ctsvc_tc_util_db_changed_cb, NULL))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_remove_changed_cb(_contacts_event._uri, ctsvc_tc_util_db_changed_cb, NULL))) break;
		return 0;
	} while (0);
	return ret;
}
...
static int _ctsvc_tc_db_get_record_event(int account_id)
{
	...
	do {
		...
		if (0 != (ret = ctsvc_tc_record_create_event(contact_id, PROPERTY_TYPE_CREATE1, &record))) break;
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_record(_contacts_event._uri, id, &record))) break;
		if (0 != (ret = ctsvc_tc_record_compare_event(record, PROPERTY_TYPE_CREATE1))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_record_destroy(record, true))) break;
		record = NULL;

		if (CONTACTS_ERROR_NONE != (ret = contacts_db_delete_record(_contacts_event._uri, id))) break;
	...
	contacts_db_delete_record(_contacts_event._uri, id);
...
static int _ctsvc_tc_db_get_records_with_query_event(int account_id)
{
	...
	do {
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_query_create(_contacts_event._uri, &query))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_filter_create(_contacts_event._uri, &filter))) break;
		if (0 != (ret = ctsvc_tc_filter_set_event(filter, PROPERTY_TYPE_CREATE1))) break;
		...
		if (0 != (ret = ctsvc_tc_record_compare_event(record_p, PROPERTY_TYPE_CREATE1))) break;
...
static int _ctsvc_tc_db_get_all_records_event(int account_id)
{
	...
	do {
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_all_records(_contacts_event._uri, OFFSET_DEFAULT, LIMIT_DEFAULT, &list))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_list_get_current_record_p(list, &record_p))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_list_destroy(list, true))) break;
		list = NULL;
...
static int _ctsvc_tc_db_get_count_event(int account_id)
{
	...
	do {
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_count(_contacts_event._uri, &count))) break;
		if (count < 1) {
			ret = -1;
			break;
		}
...
static int _ctsvc_tc_db_insert_records_event(int account_id)
{
	...
	do {
		...
		record = NULL;
		if (0 != (ret = ctsvc_tc_record_create_event(contact_id, PROPERTY_TYPE_CREATE2, &record))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_list_add(list, record))) break;
		record = NULL;
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_record(_contacts_event._uri, ids[0], &record))) break;
		if (0 != (ret = ctsvc_tc_record_compare_event(record, PROPERTY_TYPE_CREATE1))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_record_destroy(record, true))) break;
		record = NULL;

		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_record(_contacts_event._uri, ids[1], &record))) break;
		if (0 != (ret = ctsvc_tc_record_compare_event(record, PROPERTY_TYPE_CREATE2))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_record_destroy(record, true))) break;
		record = NULL;

		if (CONTACTS_ERROR_NONE != (ret = contacts_db_delete_records(_contacts_event._uri, ids, ids_count))) break;
		free(ids);
		ids = NULL;
		ids_count = 0;
	...
	contacts_record_destroy(record, true);
	contacts_list_destroy(list, true);
	contacts_db_delete_records(_contacts_event._uri, ids, ids_count);
	free(ids);
	contacts_db_delete_record(_contacts_contact._uri, contact_id);
	contacts_db_delete_record(_contacts_group._uri, group_id);
	contacts_db_delete_record(_contacts_address_book._uri, address_book_id);
...
static int _ctsvc_tc_db_update_record_event(int account_id)
{
	...
	do {
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_record(_contacts_event._uri, id, &record))) break;
		if (0 != (ret = ctsvc_tc_record_update_event(record, PROPERTY_TYPE_UPDATE1))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_update_record(record))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_record_destroy(record, true))) break;
		record = NULL;

		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_record(_contacts_event._uri, id, &record))) break;
		if (0 != (ret = ctsvc_tc_record_compare_event(record, PROPERTY_TYPE_UPDATE1))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_record_destroy(record, true))) break;
		record = NULL;
...
static int _ctsvc_tc_db_update_records_event(int account_id)
{
	...
	do {
		...
		record = NULL;
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_record(_contacts_event._uri, ids[1], &record))) break;
		if (0 != (ret = ctsvc_tc_record_update_event(record, PROPERTY_TYPE_UPDATE2))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_list_add(list, record))) break;
		record = NULL;
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_get_record(_contacts_event._uri, ids[1], &record))) break;
		if (0 != (ret = ctsvc_tc_record_compare_event(record, PROPERTY_TYPE_UPDATE2))) break;
		if (CONTACTS_ERROR_NONE != (ret = contacts_record_destroy(record, true))) break;
		record = NULL;
...
static int _ctsvc_tc_db_delete_record_event(int account_id)
{
	...
	do {
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_delete_record(_contacts_event._uri, id))) break;
		if (CONTACTS_ERROR_NO_DATA != (ret = contacts_db_get_record(_contacts_event._uri, id, &record))) {
			ret = -1;
			break;
		}
...
static int _ctsvc_tc_db_delete_records_event(int account_id)
{
	...
	do {
		...
		if (CONTACTS_ERROR_NONE != (ret = contacts_db_delete_records(_contacts_event._uri, ids, ids_count))) break;
		if (CONTACTS_ERROR_NO_DATA != (ret = contacts_db_get_record(_contacts_event._uri, ids[0], &record))) {
			ret = -1;
			break;
		}
		if (CONTACTS_ERROR_NO_DATA != (ret = contacts_db_get_record(_contacts_event._uri, ids[1], &record))) {
			ret = -1;
			break;
		}
		free(ids);
