...
%if %{?DEVICE_BUILD_TYPE_MOBILE:1}0
cp -rf src/utc/media-content/res/mobile/* %{buildroot}%{APP_PATH}%{name}/data/res/
%endif

%if %{?DEVICE_BUILD_TYPE_WEARABLE:1}0
cp -rf src/utc/media-content/res/wearable/* %{buildroot}%{APP_PATH}%{name}/data/res/
%endif

%if %{?DEVICE_BUILD_TYPE_TV:1}0
cp -rf src/utc/media-content/res/tv/* %{buildroot}%{APP_PATH}%{name}/data/res/
%endif

%if %{?DEVICE_BUILD_TYPE_TIZENIOT:1}0
cp -rf src/utc/media-content/res/tizeniot/* %{buildroot}%{APP_PATH}%{name}/data/res/
%endif
